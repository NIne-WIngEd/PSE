...
def find_analysis_dir(project_dir: Path, explicit: str | None) -> Path:
    candidates = (
        ...
        if explicit
        else [project_dir / "AFM_Web-main", project_dir]
    )
    required = [
        UNET_SCRIPT_NAME,
        UNET_MODEL_NAME,
    ...
    raise FileNotFoundError(
        "Could not find AFM_Web-main with the original U-Net, Voronoi, "
        "and ColorWheel files."
    )
...
def generate_masks(
    ...
    model, image_size, model_device = unet.load_model(
        str(analysis_dir / UNET_MODEL_NAME),
        device=str(device),
    )
...
def main() -> int:
    ...
    ensure_real_model(analysis_dir / UNET_MODEL_NAME)
    ...
    metadata = [{
        ...
        "unet_model_sha256": sha256_file(
            analysis_dir / UNET_MODEL_NAME
        ),
