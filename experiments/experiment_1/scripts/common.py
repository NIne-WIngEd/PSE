...
def resolve_project_paths(project_dir: str | Path) -> ProjectPaths:
    ...
    return ProjectPaths(
        project_dir=project,
        cnn_script=backend / "1.cnn_inference 1.py",
        cnn_weights=models / "cnn_rgb_classifier.pth",
        unet_script=backend / "2.segmentation.py",
        unet_weights=models / "best_quality_unet.pt",
        voronoi_script=backend / "2.voronoi.py",
        colorwheel_script=backend / "3.colorwheel.py",
    )


def default_project_dir() -> Path:
    # Repository root: experiments/experiment_N/scripts/common.py -> parents[3].
