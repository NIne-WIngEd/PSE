...
type AnalysisResponse = {
  ...
  metrics: MetricRow[];
  details: string;
  additional_outputs?: { label: string; url: string }[];
};

const BACKEND_URL = process.env.NEXT_PUBLIC_BACKEND_URL ?? "http://127.0.0.1:8050";
...
export default function Home() {
  ...
  const loadMaskToCanvas = useCallback((src: string) => {
    const canvas = canvasRef.current;
  ...
  useEffect(() => {
    ...
    setStrokeCount(0);
    setMaskMode("edit");
  }, [currentItem?.item_id, editedMaskMap, currentItem?.mask_image_url, loadMaskToCanvas]);
  ...
  const handleRunAnalysis = async () => {
    ...
    const editedMasksSnapshot = { ...editedMaskMap };
            ...
            <div>
              ...
              <Panel title="Image Outputs" style={{ marginBottom: 18 }}>
                ...
                >
                  ...
                  {currentAnalysis?.additional_outputs?.map((img, idx) => (
                    <ResultsImage
                      key={`${img.label}-${idx}`}
                      label={img.label}
                      src={img.url}
