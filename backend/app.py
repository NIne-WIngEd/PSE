...
MODEL_DIR = Path(os.getenv("AFM_MODEL_DIR", REPO_ROOT / "models")).expanduser().resolve()
RUNTIME_DIR = Path(os.getenv("AFM_RUNTIME_DIR", REPO_ROOT / "runtime")).expanduser().resolve()
...
CNN_WEIGHTS = MODEL_DIR / "cnn_rgb_classifier.pth"
UNET_WEIGHTS = MODEL_DIR / "best_quality_unet.pt"
...
DEVICE_REQUEST = os.getenv("AFM_DEVICE", "auto").strip().lower()
UNET_DEVICE_REQUEST = "cuda" if DEVICE_REQUEST == "auto" else DEVICE_REQUEST
...
UNET_MODEL, UNET_IMG_SIZE, UNET_DEVICE = unet_mod.load_model(
    str(UNET_WEIGHTS), device=UNET_DEVICE_REQUEST
)
...
# 4) HELPERS
# ----------------------------
def ensure_browser_preview_image(image_path: Path) -> Path:
...
def path_to_job_url(job_id: str, job_dir: Path, full_path: str) -> str:
    if not full_path:
        return ""
    ...
    try:
        ...
        backend_base = "http://127.0.0.1:8050"
        return f"{backend_base}/job_file/{job_id}/{rel.as_posix()}"
    except Exception:
        return ""
...
def pick_best_voronoi_images(voronoi_root: Path, image_stem: str):
    ...
    overlay = folder / f"{image_stem}_voronoi_overlay.png"
    morphology = folder / f"{image_stem}_morphology_map.png"

    overlay_path = str(overlay) if overlay.exists() else ""
    morphology_path = str(morphology) if morphology.exists() else ""

    return overlay_path, morphology_path
...
def analyze_one_item(item_payload: dict, edited_mask_data_url: str = "") -> dict:
    ...
    if predicted_for_ui in ("dots", "mixed"):
        ...
        if vor_results is None:
        ...
        elif isinstance(vor_results, dict):
            selected = vor_results.get("selected_images", {})

            extra1_path = selected.get("voronoi_overlay", "")
            extra2_path = selected.get("morphology_map", "")
            extra3_path = selected.get("snapshot", "")
            extra4_path = selected.get("original", "")
            ...
            extra2_note = "Morphology Map"
            extra3_note = "Snapshot"
            extra4_note = "Original"

            if not vor_results.get("ran_voronoi", False):
                extra_details_lines.append(
                    f"Voronoi skipped: {vor_results.get('reason', 'unknown reason')}"
                )
            else:
                extra_details_lines.append("Voronoi results:")

                dot_stats = vor_results.get("dot_extraction_stats", {})
                if dot_stats:
                    extra_details_lines.append(f"  Dot extraction: {dot_stats}")

                vr = vor_results.get("voronoi_results", {})
                if isinstance(vr, dict):
                    for k, v in vr.items():
                        extra_details_lines.append(f"  {k}: {v}")
                    all_metrics.extend(parse_numeric_metrics(vr))

                summary = f"{predicted_for_ui.upper()} class · Voronoi analysis applied"
        else:
            extra_details_lines.append("Voronoi: unexpected result format.")
    ...
    if predicted_for_ui in ("lines", "mixed"):
        ...
        if cw_results is None:
            ...
            # Add every extra image from the colorwheel_output folder
            if cw_dir.exists():
                for file_path in sorted(cw_dir.iterdir()):
                    if not file_path.is_file():
                        continue
                    if file_path.suffix.lower() not in {".png", ".tiff", ".tif"}:
                        continue

                    name_lower = file_path.name.lower()

                    # Skip the two primary images already shown
                    if name_lower in {"color_wheel_output.png", "one_phase_output.png"}:
                        continue

                    preview_path = ensure_browser_preview_image(file_path)

                    additional_outputs.append({
                        "label": preview_path.name,
                        "url": path_to_job_url(job_id, job_dir, str(preview_path)),
                    })
            if summary:
    ...
    return {
        ...
        "extra1_url": path_to_job_url(job_id, job_dir, extra1_path),
        "extra2_url": path_to_job_url(job_id, job_dir, extra2_path),
        "extra3_url": path_to_job_url(job_id, job_dir, extra3_path),
        "extra4_url": path_to_job_url(job_id, job_dir, extra4_path),
...
if __name__ == "__main__":
    host = os.getenv("AFM_HOST", "127.0.0.1")
    port = int(os.getenv("AFM_PORT", "8050"))
